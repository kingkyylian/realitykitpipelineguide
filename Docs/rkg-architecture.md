...
| Screenshot evidence | `verify-screenshots` reports missing/empty/invalid image evidence, requires matching JSON sidecars and runtime scene-role snapshots with enabled role and visual bounds metadata, rejects blank/solid PNG/JPEG captures, rejects duplicate visual evidence across states, applies semantic visual contracts, and accepts captured JPEG/PNG files at planned paths. |
