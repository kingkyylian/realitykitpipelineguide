...
class RkgArchetypeRuntimeTests(unittest.TestCase):
    ...
    def test_flappy_runtime_contract_is_exposed_outside_scaffold(self) -> None:
        ...
        self.assertIn("var flappyFramesElapsed: Int = 0", fields)
        self.assertIn("var isCollision: Bool = false", fields)
        self.assertIn("static let flappyGravity", rules)
        self.assertIn("static let flappyFrameInterval = 0.18", rules)
        self.assertIn("static let flappyObstacleBaseSpeed = 0.130", rules)
        self.assertIn("static func flappyObstacleSpeed(for pipesPassed: Int) -> Double", rules)
        ...
        self.assertIn("next.flappyFramesElapsed += 1", rules)
        self.assertIn("next.elapsedSeconds = Int(Double(next.flappyFramesElapsed) * flappyFrameInterval)", rules)
        self.assertIn("next.obstacleX -= flappyObstacleSpeed(for: next.pipesPassed)", rules)
