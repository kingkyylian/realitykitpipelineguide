...
3. If deliberately continuing RKG, either split `src/rkg/custom_realitykit_runtime.py` into smaller adapter sections or add a small third adapter for `collect,score,timer` or `projectile`; avoid another large inline adapter until the internal adapter list shape is clearer.
