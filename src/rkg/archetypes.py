...
ARCHETYPES: list[JsonDict] = [
    ...
    {
        "id": "flappy_side_scroller",
        "display_name": "Flappy Side Scroller",
        "mechanic": "tap to flap through scrolling obstacle gaps with gravity, collision, and pass scoring",
        "input": ["tap"],
        "camera": ["fixed_non_ar"],
        "required_asset_roles": ["player", "obstacle", "arena"],
        "optional_asset_roles": ["ui_prop", "environment"],
        "runtime_modules": RUNTIME_MODULES,
    ...
    {
        "id": "custom_realitykit",
        "display_name": "Custom RealityKit Skeleton",
        "mechanic": "compose a RealityKit skeleton from camera, input, gameplay systems, and asset roles",
        "input": ["tap", "drag", "tilt_tap", "dual_stick", "gamepad_touch", "tap_swipe"],
        "camera": ["fixed_non_ar", "chase", "first_person", "third_person", "top_down"],
        "required_asset_roles": ["player", "arena"],
        ...
        "runtime_modules": RUNTIME_MODULES,
        "screenshot_states": ["gameplay_start", "mid_action", "fail_or_hit", "results"],
        "screenshot_proofs": {
            "gameplay_start": "Tap Start; state or skeleton UI is visible; player and arena roles are loaded.",
            "mid_action": "Trigger the primary input; the generated skeleton shows active gameplay roles.",
            "fail_or_hit": "Exercise collision, hit, or fail proxy state; hazard/enemy/cover roles are visible when declared.",
            "results": "End or reset the session; result or reset UI is visible.",
        },
        "scope_risk": "medium",
    },
]
