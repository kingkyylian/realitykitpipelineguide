...
class ReleaseDocsTests(unittest.TestCase):
    def test_changelog_has_v020_draft_above_v010_history(self) -> None:
        changelog = read("CHANGELOG.md")

        self.assertIn("## Unreleased", changelog)
        self.assertIn("## v0.2.0 - RKP product path and RKG labs preview (draft, 2026-05-10)", changelog)
    ...
    def test_release_draft_is_actionable_without_claiming_publication(self) -> None:
        release = read("Docs/releases/v0.2.0.md")

        self.assertIn("Status: Draft", release)
        self.assertIn("Target tag: `v0.2.0`", release)
        self.assertIn("Compare range: `v0.1.0..HEAD`", release)
        self.assertIn("No tag, GitHub Release, or push has been made from this draft.", release)
        self.assertIn("- [ ] Push `main` only after explicit user approval.", release)
        self.assertIn("- [ ] Create `v0.2.0` tag only after release approval.", release)
        self.assertIn("RKP-first product boundary", release)
        self.assertIn("RKG remains experimental labs", release)

    def test_showcase_uses_next_release_draft_not_old_first_release(self) -> None:
        showcase = read("Docs/github-showcase.md")

        self.assertIn("## Next Release Draft", showcase)
    ...
    def test_release_checklist_points_to_next_release_candidate(self) -> None:
        checklist = read("Docs/repo-release-checklist.md")

        self.assertIn("Next release candidate: `v0.2.0`", checklist)
        ...
        self.assertIn("Create `v0.2.0` release from `Docs/releases/v0.2.0.md`", checklist)
