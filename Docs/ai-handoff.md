...
3. If deliberately continuing RKG, use `rkg list-adapters --json` to choose gaps from the current adapter matrix; the next useful slice is either screenshot capture evidence for projectile generated projects or a new small adapter such as `physics`/`enemy_ai`, not another broad rewrite.
