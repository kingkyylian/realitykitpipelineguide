...
3. If deliberately continuing RKG, use `Docs/rkg-shard-volley-dogfood.md` as the current gap list. Adapter-specific `qa-plan` proof text and the first blank/solid PNG screenshot guardrail are done; the next useful slice is idea-to-project orchestration, JPEG/role-presence visual QA, or the generated asset-brief to RKP asset-command bridge.
