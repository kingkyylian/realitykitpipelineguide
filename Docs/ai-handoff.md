...
3. If deliberately continuing RKG, use `Docs/rkg-shard-volley-dogfood.md` plus the Sprint 139 Flappy evidence and Sprint 142 QA gates as the current gap list. Adapter-specific `qa-plan` proof text, blank/solid PNG/JPEG guardrails, malformed JPEG rejection, duplicate visual evidence detection, capture-contract sidecars, runtime scene-role snapshots, runtime scene-role visibility metadata, minimum visual bounds metadata, semantic top-panel/flat-scene/too-dark/bottom-control-occlusion/center-modal-occlusion QA, idea-to-project orchestration, generated asset-brief to RKP command planning, one emitted asset task dogfood, `accept-first-asset`, `accept-assets`, fresh capture XcodeGen generation, local CLI subprocess module dispatch, the readable `target_proxy` bullseye asset, full five-asset Shard Volley demo acceptance, first game-shell/world-rig presentation parity, SceneEvents-based idle motion, native Flappy-like scaffold proof, and Flappy auto-loop/speed-ramp proof are done. The next useful slice is pixel-level mesh-visibility QA or richer Flappy/charge/launch VFX, sound, haptics, and level variety hooks.
