...
| Shard Volley RKG dogfood | Verified | `Docs/rkg-shard-volley-dogfood.md`, `Docs/screenshots/rkg_shard_volley_*.jpg`; idea score, `new-game`, `validate-spec`, `plan-game`, `qa-plan`, `init-game`, `verify-game`, `capture-screenshots`, and `verify-screenshots` were run end-to-end for a projectile/shooting/score game. Dogfood found and fixed collector-score adapter overlap and too-short screenshot launch wait |
...
3. If deliberately continuing RKG, use `Docs/rkg-shard-volley-dogfood.md` as the current gap list. Adapter-specific `qa-plan` proof text, blank/solid PNG/JPEG guardrails, malformed JPEG rejection, duplicate visual evidence detection, and capture-contract sidecars are done; the next useful slice is idea-to-project orchestration, runtime scene-role visibility QA, or the generated asset-brief to RKP asset-command bridge.
