...
3. If deliberately continuing RKG, refactor custom system adapters out of the growing generator strings before adding a third adapter. Racing and FPS/shooter adapters are complete first slices; the next behavior adapter can be collect/score/timer or projectile once generator ownership is split.
