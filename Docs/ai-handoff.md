...
3. If deliberately continuing RKG, use `Docs/rkg-shard-volley-dogfood.md` as the current gap list. Adapter-specific `qa-plan` proof text, blank/solid PNG/JPEG guardrails, malformed JPEG rejection, and duplicate visual evidence detection are done; the next useful slice is idea-to-project orchestration, semantic role-presence QA, or the generated asset-brief to RKP asset-command bridge.
