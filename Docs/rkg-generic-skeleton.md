...
The first two system adapters are racing and FPS/shooter. They prove vehicle lane movement, checkpoint/lap progress, collision/result state, first-person aim/fire state, enemy/health/cover proof, and camera rig entity binding. Their state/rule/UI/scene generator surface now lives in `src/rkg/custom_realitykit_runtime.py`, so the next runtime slice can either split that module into smaller adapter sections or add a small third adapter for collect/score/timer or projectile behavior. Generated games are still skeletons, not genre-complete gameplay.
