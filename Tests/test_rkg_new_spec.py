...
class RkgNewSpecTests(unittest.TestCase):
    ...
    def test_new_spec_writes_valid_fighter_game_spec(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(spec["quality"]["feel"]["movement"], "snappy")
            self.assertEqual(spec["quality"]["feedback"]["hit"], ["hit_spark", "camera_kick", "haptic_hook"])
            self.assertEqual(spec["quality"]["style"]["shape_language"], "readable_duel_silhouettes")
    ...
    def test_new_spec_writes_valid_flappy_game_spec(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(spec["quality"]["feel"]["movement"], "floaty")
            self.assertEqual(spec["quality"]["feedback"]["fail"], ["collision_flash", "freeze_frame", "result_transition"])
            self.assertEqual(spec["quality"]["style"]["palette"], "bright_readable_lanes")
