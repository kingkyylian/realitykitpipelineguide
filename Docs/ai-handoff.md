...
3. If deliberately continuing RKG, add a small third `CustomRealityKitRuntimeAdapter` for `collect,score,timer` or `projectile`; keep it registry-driven and verify both generated Swift build and screenshot-state behavior before claiming gameplay readiness.
