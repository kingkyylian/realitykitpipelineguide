...
- The first role-footprint gate adds `role_visibility_contract.min_visual_extent` to `qa-plan`. `verify-screenshots` now rejects enabled expected roles with microscopic runtime visual bounds as `scene_role_not_visible`, which closes the gap where a role node existed but its rendered footprint was too small to be meaningful.
- The first opt-in role-pixel gate adds `role_pixel_contract` to `qa-plan`. When a sidecar provides normalized `role_pixel_evidence` regions and the contract is required, `verify-screenshots` samples those screenshot regions and rejects missing, malformed, undersampled, or flat role evidence.

## Remaining Gaps For A Comprehensive Tool

1. Screenshot verification has a first semantic visual contract, a minimum runtime role-footprint contract, and an opt-in role-pixel evidence contract. It now catches blank/solid PNG/JPEG evidence, duplicate state captures, missing/mismatched sidecars, declared-role metadata mismatches, missing/mismatched runtime scene-role snapshots, disabled expected scene roles, malformed role position metadata, missing/zero/tiny visual bounds metadata, missing/flat role-pixel evidence regions, debug-overlay-like top panels, bottom control-panel occlusion, center modal occlusion, flat scene bands, and too-dark scene bands. It still does not automatically project each declared mesh into pixels or perform OCR-level text-overlap analysis.
...
- Extend screenshot QA by generating role-pixel evidence regions automatically from runtime/camera metadata, then add OCR-level text-overlap checks.
