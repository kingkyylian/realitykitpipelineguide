...
- Game specs.
- Game feel, feedback, and art-direction quality contracts.
- Archetype templates.
...
- Game idea scoring.
- GameSpec validation.
- Quality contract planning for movement feel, score/hit/fail feedback hooks, and art-direction readability.
- Project scaffolding.
- Asset role mapping.
...
`capture-screenshots` generates the Xcode project from `project.yml` when needed, builds the generated project, installs it on a simulator, launches each screenshot state, and writes the capture files. It also writes `Docs/screenshots/<state>.json` sidecars that record the game id, state, screenshot-state case, expected visible roles, proof cue, automation hint, screenshot path, and runtime scene snapshot path. The running generated app writes `Documents/rkg-scene-snapshot-<state>.json`; capture copies it to `Docs/screenshots/<state>.scene.json`. `verify-screenshots` then checks that every QA plan capture path exists under the generated project, has a supported JPEG or PNG image with readable dimensions, has a matching sidecar, and has runtime scene-role evidence for the expected roles. Expected roles must be backed by enabled `rkg|...` entities with finite position metadata and `visual_bounds.extents` that meet the QA plan's `role_visibility_contract.min_visual_extent`; disabled, zero-bound, or microscopic expected roles fail as `scene_role_not_visible`. The optional `role_pixel_contract` can require sidecar `role_pixel_evidence` regions for expected roles; flat or malformed evidence regions fail as `missing_role_pixel_evidence`, `invalid_role_pixel_evidence`, or `role_pixel_not_visible`. PNG captures are sampled directly, and JPEG captures are sampled through the macOS `sips` rasterizer when available. Near-solid or blank evidence is rejected as `blank_or_solid`; repeated visual evidence across different release states is rejected as `duplicate_visual_evidence`. First-pass semantic screenshot failures are reported as `semantic_debug_overlay`, `semantic_control_occlusion`, `semantic_center_occlusion`, `semantic_flat_scene`, or `semantic_scene_too_dark`.
